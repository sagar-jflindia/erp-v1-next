...
import { Plus, RefreshCw, Edit3, Trash2, CheckCircle, X, Settings2, History } from "lucide-react";
...
export default function StockAdjustmentPage() {
  ...
  const [params, setParams] = useState({
    ...
    fromDate: defaultDates.from, toDate: defaultDates.to, sortKey: "adjustment_id", sortDir: "desc"
  });
  ...
  const fetchData = useCallback(async (isLoadMore = false) => {
    ...
    try {
      ...
      const body = await stockAdjustmentService.getAll(apiParams);
  ...
  const HEADERS = [
    ...
    ["Item Details", "item_dcode", (v, row) => (
      <div className="flex flex-col leading-tight min-w-[150px]">
        <span className="font-bold text-slate-800 uppercase text-[11px] tracking-tight">{v || "N/A"}</span>
        <span className="text-slate-400 font-mono text-[9px]">{row.item_code || "—"}</span>
      </div>
    ), { width: "200px" }],

    ["Adjustment", "qty", (v, row) => (
    ...
    ["Created At", "created_at", (v) => (
      <div className="flex flex-col leading-tight">
        <span className="text-slate-700 font-medium text-[10px]">{dayjs(v).format("DD/MM/YYYY")}</span>
  ...
  return (
    <div className="flex flex-col h-full md:h-[calc(100vh-140px)] w-full bg-slate-100 md:overflow-hidden">
      <div className="bg-white border border-slate-300 flex flex-col flex-1 min-h-0 rounded-none shadow-sm overflow-hidden">
        ...
        <div className="px-3 py-2 bg-white border-b border-slate-200 flex flex-col gap-2 shrink-0">
          ...
          {selected && (
            <div className="flex items-center justify-between px-3 py-1.5 bg-indigo-50 border border-indigo-100">
              <span className="text-[10px] font-bold text-indigo-600 uppercase">Selected: ADJ-#{selected} {selectedRecord?.item_code}</span>
        ...
        <div className="px-3 md:px-4 py-3 bg-slate-50 border-b border-slate-200 shrink-0">
          <DateRangeFilter 
            ...
            searchPlaceholder="Search item or ID..."
      ...
      <StockAdjustmentModal open={modalOpen} onClose={() => { setModalOpen(false); setEditItem(null); }} onSuccess={() => { fetchData(); setSelected(null); }} editData={editItem} mode={modalMode} />
      <DeleteModal item={deleteItem} onClose={() => setDeleteItem(null)} onSuccess={() => { fetchData(); setSelected(null); }} service={stockAdjustmentService} entityLabel="Adjustment Record" idKey="adjustment_id" />
    </div>
  );
}